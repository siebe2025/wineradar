    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Your generated wine-market briefings</p>
        </div>
          ...
          <div className="space-y-2">
            {briefings.map((b) => (
              <Link
                key={b.id}
                href={`/briefings/${b.id}`}
                className="card block hover:border-wine-300 transition-colors"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="font-medium text-[#111] truncate">{b.title}</p>
                    {b.summary && (
                      <p className="text-sm text-gray-500 mt-0.5 line-clamp-1">{b.summary}</p>
                    )}
