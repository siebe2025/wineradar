    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div>
          <Link
            href="/briefings"
            className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-[#111] mb-4 transition-colors"
          >
          ...
          <h1 className="text-xl font-semibold text-[#111]">{briefing.title}</h1>
        ...
        {/* Summary banner */}
        {briefing.summary && (
          <div className="rounded-lg border-l-4 border-wine-700 bg-wine-50 px-5 py-4">
              ...
              return (
                <div key={item.id} className="card space-y-4">
                  ...
                  {content ? (
                    <>
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1.5">Overview</p>
                        <p className="text-sm text-[#374151] leading-relaxed">{content.summary}</p>
                      </div>

                      {content.key_developments?.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Key developments</p>
                      ...
                      {content.why_it_matters && (
                        <div className="rounded-md bg-amber-50 border border-amber-100 px-4 py-3">
                          <p className="text-xs font-semibold uppercase tracking-wider text-amber-600 mb-1">Why it matters</p>
                          <p className="text-sm text-[#374151]">{content.why_it_matters}</p>
                        </div>
                      )}

                      {content.commercial_implication && (
                        <div className="rounded-md bg-blue-50 border border-blue-100 px-4 py-3">
                          <p className="text-xs font-semibold uppercase tracking-wider text-blue-600 mb-1">Account manager implication</p>
                          <p className="text-sm text-[#374151]">{content.commercial_implication}</p>
                        </div>
                      )}
                    </>
                  ) : (
                      ...
                      <div className="space-y-1">
                        {sources.map((s, i) => (
                          <a
                            ...
                            className="block text-xs text-wine-700 hover:underline truncate"
                          >
                            {s.title || s.url}
                            {s.published_date && (
                              <span className="text-gray-400 ml-1">({s.published_date.slice(0, 10)})</span>
                            )}
                          </a>
                        ))}
                      </div>
